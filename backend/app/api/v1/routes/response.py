...
@router.post("/actions/{action_id}/rollback", response_model=ActionOut)
async def rollback_action(
    action_id: str,
    db: AsyncSession = Depends(get_db),
    user: CurrentUser = Depends(require_permission(Permission.RESPONSE_EXECUTE)),
):
    from app.modules.response.application.rollback_service import RollbackService

    return await RollbackService(db).rollback(action_id, actor_id=user.id)
